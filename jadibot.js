  ...
  try {
    const start = async() => {
      ...
      conn.ev.on("connection.update", async up => {
        ...
        if(connection == "connecting") return await msg.reply("_*Conectando al servidor...*_");
        if(connection){
          if (connection != "connecting") console.log("Conectando al servidor..")
        }
        console.log(up)
        if(up.qr) await sendFile(msg.from, await qrcode.toDataURL(up.qr,{scale : 8}),"", 'Escanea este QR para convertirte en un bot temporal\n\n1. Haga clic en los tres puntos en la esquina superior derecha\n2. Toque WhatsApp Web\n3. Escanea este QR \nEl QR Caduca en 20 segundos', msg)
	  ...
	  sendMessage(config.owner[0],{text: txt,  withTag : true})
        ...
        if(connection == "close") {
          ...
          if (reason === DisconnectReason.restartRequired) {
            msg.reply("_*Reinicio requerido, reiniciando...*_")
            start();
