...
module.exports = {
	...
	async run({ msg, conn }) {
		...
		txt += monospace(`   × Éxito : ${require("mathjs").evaluate(success.join("+"))}`) + `\n`
		txt += monospace(`   × Fallidos : ${require("mathjs").evaluate(failed.join("+"))}`) + "\n\n";
		let dbny = dashboard.length > 5 ? 5 : dashboard.length;
		for(let i = 0; i < dbny; i++){
		  txt += `*乂 Comandos : ${dashboard[i].name}*\n`
		  txt += monospace(`   × Total : ${dashboard[i].success + dashboard[i].failed}`) + `\n`;
		  txt += monospace(`   × Cmd Con Éxito : ${dashboard[i].success}`) + `\n`; 
		  txt += monospace(`   × Cmd Fallidos : ${dashboard[i].failed}`) +`\n`;
		  txt += monospace(`   × Utilizado por última vez : ${await prettyms(Date.now() - dashboard[i].lastUpdate, {
				verbose: true,
			})}`) + `\n\n`;
		}
