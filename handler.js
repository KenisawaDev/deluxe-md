...
module.exports = handler = async (mek, conn, map) => {
  ...
  if(isGroup){
	...
	global.respon = {
	  ...
	  success: "Listo ✓",
	  error:{
	    cmd: "Lo sentimos error de comando, por favor póngase en contacto con el propietario!!",
	    lv: ["¡¡Enlace inválido!!","donde esta el enlace?"],
	    api: "¡Lo siento, el sistema está ocupado!"
	  },
	  group: "¡Este comando se puede usar en grupos!",
	  private: "Este comando se puede usar en el chat privado/PV del Bot..",
	  ...
	  owner: "Este comando es solo para Dueños!!"
	}
	...
	global.error = (command, e, msg) => {
	  ...
	  error += "*Error Log*\n"
	  error += String(e)
	  if(String(e).includes("No se puede leer la propiedad 'datos' de indefinido")) return msg.reply('no se encontraron medios, vuelva a enviar los medios')
	...
	if(options.isQuoted && !msg.quoted) {
			await msg.reply(`Por favor, responda a un mensaje`);
			return true;
	}
	...
	if(options.isMedia) {
			...
			if(typeof medianya[0] != "undefined" && !medianya.includes(msg.quoted ? msg.quoted.mtype : []))
				return msg.reply(`Por favor responda *${medianya.map((a) => `${((aa = a.charAt(0).toUpperCase()), aa + a.slice(1).replace(/message/gi, ""))}`).join("/")}*`);
		}
