...
async function start(){
  ...
  //console.log(color(figlet.textSync('Deluxe', { font: 'doom', horizontalLayout: 'default' })))
  console.log(color('------------------------------------------------------------------------', 'white'))
  console.log(color('[CREADOR]', 'aqua'), color(config.author, 'magenta'))
  console.log(color('[BOT]', 'aqua'), color('El bot esta listo!', 'magenta'))
	...
	conn.ev.on("call", async (senku) => {
	  ...
	  for(let sen of senku){
	    if(sen.isGroup == false){
	      if(sen.status == "offer"){
	        teks = `*${conn.user.name}* no puedo recibir llamadas ${sen.isVideo ? `video` : `suara`}. ¡Lo siento, serás bloqueado! Si es por accidente, póngase en contacto con el propietario!\n\n Número de propietario :\n${config.owner.map((a) => `${a.split(`@`)[0]} | ${conn.getName(a).includes("+62") ? "No detectado" : conn.getName(a)
							}`).join("\n")}`
