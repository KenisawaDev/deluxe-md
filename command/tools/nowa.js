module.exports = {
    ...
    query: "<número>",
    category: "tools",
    desc: `Ingresa tu número!\nEjemplo : #nowa 5493865xxxxxx`,
    async run({msg,conn}, {q}){
      const moment = require('moment-timezone');
            var teks = q
            if (!teks.includes('x')) return msg.reply('que?')
            msg.reply('espere...')
              ...
              try {
               try {
                ...
                if (anu1 == '401' || anu1.status.length == 0) {
                  ...
                  nomerny += `@${anu[0].jid.split("@")[0]}\n Recado : ${anu1.status}\n Fecha de cambio : ${moment(anu1.setAt).tz('America/Buenos_Aires').format('ddd DD MMM YYYY')}\n\n`
