	  ...
	  try {
	    switch(cmdNya){
	      case "join":
	        ...
	        txt += " • De : " + msg.from + "\n"
	        txt += " • Enlace de grupo : " + q
          reply("*Listo, espera a que el propietario acepte el enlace :)*")
          return conn.sendMessage(config.owner[0], {
                 text: txt,
                 footer: global.footer,
                 buttons: [ 
                     { buttonId: `.accgc ${q} ${msg.from} ${msg.key.id}`, buttonText: { displayText: 'Aceptar (Join)' }, type: 1 },
                     { buttonId: `.rejectgc ${msg.from} ${msg.key.id}`, buttonText: { displayText: 'Rechazar (Reject)' }, type: 1 }
                   ],
                  headerType: 1
          ...
          conn.sendMessage(args[0], { text: `_Habla con mi dueño si quieres que me una_` }, { quoted: w });
